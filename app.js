...
// Expande os dossiês com pistas adicionais sem remover as pistas originais.
const extraClues = {
  1: ["Registro de sessão", "A sessão que restaurou o arquivo começou dois minutos antes do evento."],
  2: ["Dispositivo externo", "O dispositivo externo manteve uma sessão ativa após o encerramento do contrato."],
  3: ["Telemetria", "A queda foi iniciada manualmente e terminou sem falha de energia registrada."],
  4: ["Origem do token", "O token antigo foi reutilizado a partir de um equipamento não reconhecido."],
  5: ["Horário oculto", "A sequência também corresponde a um intervalo específico da estação central."],
  6: ["Ruído de fundo", "O áudio contém uma marca temporal que coincide com o acesso ao sistema."],
  7: ["Padrão final", "As evidências dos casos anteriores formam uma mesma cadeia de eventos."]
};
cases.forEach(c => {
  const extra = extraClues[c.id];
  if (extra && !c.clues.some(clue => clue[0] === extra[0])) c.clues.push(extra);
});
...
function decide(i){let c=current();if(state.solved.includes(c.id))return;const correct=i===c.correct;state.decisions[c.id]=i;state.solved.push(c.id);addXP(50,"Caso concluído");log(`Decisão final no caso ${c.id}: ${c.options[i]}`);if(correct){addXP(30,"Linha de investigação coerente");}else{log("O jogador errou a conclusão e recebeu a resposta correta no relatório.");}save();openModal(`<h3>Relatório final — ${c.code}</h3><p>Você escolheu: <strong>${c.options[i]}</strong>.</p><p class="result-badge ${correct?"success":"error"}">${correct?"✅ Você acertou a conclusão!":"❌ Você errou a conclusão."}</p><p><strong>Resposta correta:</strong> ${c.options[c.correct]}</p><p>${correct?"Sua conclusão está alinhada com as pistas principais.":"A resposta correta é apresentada acima. Revise as evidências para entender a solução."}</p><button class="primary" id="nextBtn">Continuar</button>`);$("#nextBtn").onclick=()=>{closeModal();go("cases");};}
