    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (signupMode && !result.data.session) {
          // Após o cadastro, o jogador permanece na tela de autenticação,
          // mas volta automaticamente para o modo "Entrar".
          signupMode = false;
          submit.textContent = 'Entrar';
          mode.textContent = 'Criar conta';
          password.value = '';
          password.autocomplete = 'current-password';
          setMessage(message, 'Conta criada! Verifique seu e-mail. Depois de confirmar, volte para esta página e faça login.');
          email.focus();
          return;
        }
  ...
  function loadSupabase() {
    ...
    script.onload = async () => {
      try {
        ...
        // Quando o usuário retorna do link de confirmação, mostramos a tela
        // inicial no modo login. O usuário decide quando entrar no jogo.
        ...
        if (returnedFromConfirmation) {
          ...
          confirmationMessage.className = 'cyber-auth-message';
          confirmationMessage.textContent = 'E-mail confirmado! Agora faça login para iniciar sua investigação.';
          confirmationMessage.style.color = '#1bcaff';
          document.getElementById('cyberMessage')?.replaceWith(confirmationMessage);
