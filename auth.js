...
(() => {
  ...
  function injectStyles() {
    ...
    style.textContent = css + `
      /* Atualização Forge Studios: sem menu lateral duplicado */
      #cyberWelcome .cd-shell { grid-template-columns: minmax(0, 1fr); }
      #cyberWelcome .cd-sidebar { display: none !important; }
      #cyberWelcome .cd-topbar, #cyberWelcome .cd-main, #cyberWelcome .cd-footer { grid-column: 1; }
      #cyberWelcome .cd-topbar { justify-content: space-between; flex-wrap: wrap; }
      #cyberWelcome .cd-topnav { flex-wrap: wrap; }
      #cyberWelcome .cd-info-overlay { display:none; position:fixed; inset:0; z-index:100001; background:rgba(0,3,8,.86); padding:20px; overflow:auto; }
      #cyberWelcome .cd-info-overlay.open { display:grid; place-items:center; }
      #cyberWelcome .cd-info-card { width:min(760px,100%); max-height:90vh; overflow:auto; padding:28px; border:1px solid #16caff; border-radius:18px; background:linear-gradient(145deg,#082039,#020b14); box-shadow:0 0 50px rgba(0,180,255,.2); }
      #cyberWelcome .cd-info-card h2 { color:#16caff; }
      #cyberWelcome .cd-info-card p, #cyberWelcome .cd-info-card li { color:#c0d2e1; line-height:1.75; }
      #cyberWelcome .cd-info-close { float:right; border:1px solid #42627a; border-radius:50%; background:transparent; color:white; width:38px; height:38px; font-size:22px; }
      #cyberWelcome .cd-case-mini { border:1px solid rgba(22,202,255,.35); padding:14px; border-radius:12px; margin:10px 0; }
    `;
  ...
  function openInfoModal(type) {
    const overlay = document.getElementById('cdInfoOverlay');
    const content = document.getElementById('cdInfoContent');
    if (!overlay || !content) return;
    const data = {
      about: `<p class="cd-kicker">FORGE STUDIOS // NOSSA HISTÓRIA</p><h2>Uma ideia criada por um casal gaúcho</h2><p>A Forge Studios nasceu da união de um casal gaúcho apaixonado por tecnologia, criatividade e jogos independentes. Entre conversas, estudos e protótipos, surgiu o sonho de criar experiências próprias e contar histórias marcantes.</p><p>O Cyber Detective nasceu dessa vontade: um jogo de investigação digital no qual o jogador analisa pistas, conecta evidências e toma decisões para solucionar mistérios do universo tecnológico.</p><p><strong>Nosso propósito:</strong> transformar curiosidade em investigação e cada pista em uma nova descoberta.</p>`,
      cases: `<p class="cd-kicker">ARQUIVO CONFIDENCIAL</p><h2>Casos disponíveis no jogo</h2><div class="cd-case-mini"><strong>CASE-001 — O Arquivo Fantasma</strong><p>Um arquivo apagado reaparece em servidores diferentes.</p></div><div class="cd-case-mini"><strong>CASE-002 — O Cofre de Vidro</strong><p>Uma empresa perde o acesso a um cofre digital.</p></div><div class="cd-case-mini"><strong>CASE-003 — A Cidade Sem Sinal</strong><p>Uma região fica sem comunicação por 18 minutos.</p></div><div class="cd-case-mini"><strong>CASE-004 — O Perfil Duplicado</strong><p>Uma identidade digital é copiada para aplicar golpes.</p></div><div class="cd-case-mini"><strong>CASE-005 — O Código da Estação</strong><p>Uma mensagem escondida indica uma entrega clandestina.</p></div><div class="cd-case-mini"><strong>CASE-006 — O Cofre de Voz</strong><p>Uma gravação contém informações escondidas.</p></div><div class="cd-case-mini"><strong>CASE-007 — A Última Conexão</strong><p>Uma investigação reúne pistas de toda a campanha.</p></div>`,
      help: `<p class="cd-kicker">CENTRAL DE SUPORTE</p><h2>Fale com a Forge Studios</h2><p>Encontrou um problema ou quer enviar uma sugestão?</p><p>📧 <strong>E-mail:</strong> <a href="mailto:forgestudiosofc@gmail.com">forgestudiosofc@gmail.com</a></p><p>📸 <strong>Instagram:</strong> <a href="https://instagram.com/forge.studios_ofc" target="_blank" rel="noopener">@forge.studios_ofc</a></p><p class="muted">Você pode substituir esses contatos pelos canais oficiais da equipe.</p>`
    };
    content.innerHTML = data[type] || data.about;
    overlay.classList.add('open');
  }

  function closeInfoModal() {
    document.getElementById('cdInfoOverlay')?.classList.remove('open');
  }
...
<header class="cd-topbar">
          <nav class="cd-topnav" aria-label="Navegação superior">
            <button class="active" id="cdHomeButton" type="button">Início</button>
            <button id="cdAboutButton" type="button">Sobre</button>
            <button id="cdCasesButton" type="button">Casos</button>
            <button id="cdHelpButton" type="button">Ajuda</button>
          </nav>
          <div class="cd-top-actions">
            <button class="cd-btn" id="cdSoundButton" type="button">🎧 &nbsp; Som de suspense</button>
        ...
        <div class="cd-info-overlay" id="cdInfoOverlay" role="dialog" aria-modal="true">
          <section class="cd-info-card">
            <button class="cd-info-close" id="cdInfoClose" type="button" aria-label="Fechar">×</button>
            <div id="cdInfoContent"></div>
          </section>
        </div>
    ...
    document.getElementById('cdHomeButton')?.addEventListener('click', () => {
      closeInfoModal();
      document.querySelector('#cyberWelcome .cd-topnav .active')?.classList.remove('active');
      document.getElementById('cdHomeButton')?.classList.add('active');
    });
    document.getElementById('cdAboutButton')?.addEventListener('click', () => openInfoModal('about'));
    document.getElementById('cdCasesButton')?.addEventListener('click', () => openInfoModal('cases'));
    document.getElementById('cdHelpButton')?.addEventListener('click', () => openInfoModal('help'));
    document.getElementById('cdInfoClose')?.addEventListener('click', closeInfoModal);
    document.getElementById('cdInfoOverlay')?.addEventListener('click', event => {
      if (event.target.id === 'cdInfoOverlay') closeInfoModal();
    });
    ...
    document.getElementById('cdSoundButton')?.addEventListener('click', () => {
      const button = document.getElementById('cdSoundButton');
      if (button.dataset.enabled === '1') {
        window.dispatchEvent(new Event('cyber:suspense-stop'));
        button.dataset.enabled = '0';
        button.textContent = '🎧  Som de suspense';
      } else {
        window.dispatchEvent(new Event('cyber:suspense-start'));
        button.dataset.enabled = '1';
        button.textContent = '🔇  Desativar suspense';
      }
    });
  ...
  // Compatibilidade: o botão pode estar no jogo ou ser criado por uma versão antiga.
  document.addEventListener('click', event => {
    if (event.target && event.target.id === 'logoutBtn') {
      document.dispatchEvent(new CustomEvent('cyber:logout-request'));
    }
  });
  ...
  async function submitAuth(event) {
    ...
    try {
      ...
      if (signupMode && !result.data.session) {
        ...
        document.getElementById('cdAuthSubtitle').textContent = 'Sua conta foi criada. Agora faça login.';
        ...
        setMessage(message, 'Conta criada! Agora você pode fazer login.');
        return;
      }
  ...
  function loadSupabase() {
    ...
    script.onload = async () => {
      try {
        ...
        // A tela inicial deve aparecer sempre. Assim, o jogador nunca entra
        // diretamente no jogo sem visualizar Entrar/Criar conta/Jogar sem conta.
        // A sessão existente não remove o menu inicial.
        buildInterface();
  ...
  // O jogo dispara este evento quando o jogador clica em "Sair".
  // Encerramos a sessão e recarregamos a página para voltar ao menu inicial.
  document.addEventListener('cyber:logout-request', async () => {
    try {
      if (supabaseClient) await supabaseClient.auth.signOut();
    } catch (error) {
      console.error('[Cyber Detective Auth] Logout:', error);
    } finally {
